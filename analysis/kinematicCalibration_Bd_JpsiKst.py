#!/usr/bin/env python
"""
Script to analyze the B -> J/psi K* sample to get the B pt, eta, and additional
track corrections.
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(description='Script used to calibrate Bd kin', epilog='Test example: ./kinematicCalibration_Bd_JpsiKst.py', add_help=True)
    parser.add_argument('-c', '--category', type=str, default='high', choices=['low', 'mid', 'high'], help='Category.')
    parser.add_argument('--version', '-v', default='test', help='Version.')
    parser.add_argument('--skim-tag', type=str, default='', help='Tag to append at the name of the skimmed files directory')
    parser.add_argument('--show-plots', default=False, action='store_true', help='Show plots by setting ROOT batch mode OFF (default ON)')
    parser.add_argument('--bscal', default=False, type=str2bool, help='Apply beam spot calibration')
    parser.add_argument('--draw-precal', default=False, action='store_true', help='Draw also precal plots')
    ...
    # Load MC
    mcSample = DSetLoader('Bd_JpsiKst_General', candDir='ntuples_Bd2JpsiKst_%s' % NTUPLE_TAG, skim_tag=args.skim_tag)
    ...
    dsetRD_loc = dataLoc+'cmsRD/skimmed'+args.skim_tag+'/B2JpsiKst_{}_{}_corr.root'.format(NTUPLE_TAG,cat.name)
    ...
    dfMC['PV_x_diff'] = (dfMC['vtx_PV_x'] - dfMC['beamSpot_x'])*10000
    dfMC['PV_y_diff'] = (dfMC['vtx_PV_y'] - dfMC['beamSpot_y'])*10000
    dfRD['PV_x_diff'] = (dfRD['vtx_PV_x'] - dfRD['beamSpot_x'])*10000
    dfRD['PV_y_diff'] = (dfRD['vtx_PV_y'] - dfRD['beamSpot_y'])*10000
    makePlot('PV_x_diff', binning=[100, -100, 100], wMC=[dfMC['w']], axis_title=[r'PV - beamspot (x) (#mu m)', 'Normalized entries'], saveFig='PV_x_diff_'+cat.name+'.png')
    makePlot('PV_y_diff', binning=[100, -100, 100], wMC=[dfMC['w']], axis_title=[r'PV - beamspot (y) (#mu m)', 'Normalized entries'], saveFig='PV_y_diff_'+cat.name+'.png')
